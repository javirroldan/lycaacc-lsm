...
- **Informes PDF por sección** (**solo admin**): botón "Informe PDF" en cada sección (Formaciones **azul**, Locomotoras **verde**, Lavado **naranja**) que abre un modal de **rango de fechas** (`DateRangeModal`) con **Desde / Hasta**; si quedan vacíos se genera el informe completo. Filtra los datos por **fecha de cada lavado** y el PDF agrega la línea **"Período: desde — hasta"** debajo de la fecha de generación, con el box **RESUMEN** (semáforo) y **una sola tabla agrupada** (sin separar resumen/acumulado): por cada formación/locomotora —en orden numérico ascendente (1→23 / nombre)— sus lavados del período ordenados **de más reciente a más antiguo**. El **N°/nombre figura solo en la primera fila** del grupo (en negrita, junto a los Días y Estado actuales de la card) y las filas siguientes muestran solo fecha y situación (+ **Servicio** en Locomotoras; + **ingreso/egreso/pasadas/resultado** en Lavado). Cada grupo alterna una **banda de color sutil** para separar y leer los bloques de un vistazo. Compatible con `navigator.share` y fallback a descarga.
...
src/
  ...
  lib/
    ...
    report.ts               Generadores de PDF por sección con rango de fechas
                            ...
                            Una sola tabla agrupada por formación/locomotora
                            (N° solo en la primera fila, fechas descendentes,
                            bandas alternadas por grupo); descarga y share
  components/
